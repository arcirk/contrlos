...
#ifndef CONTROLSPROG_NETWORK_HPP
#define CONTROLSPROG_NETWORK_HPP
                ...
                for (AddrList addr = ifaddrs; addr != 0; addr = addr->Next)
                {
                    ...
                    if (addr->Ipv4Enabled) {
                        for (Addr *uaddr = addr->FirstUnicastAddress; uaddr != 0; uaddr = uaddr->Next) {
                            ...
                            res.emplace_back(boost::asio::ip::make_address_v4(ntohl(reinterpret_cast<sockaddr_in *>(uaddr->Address.lpSockaddr)->sin_addr.s_addr)));
                        }
                    }
                    ...
                    if (addr->Ipv6Enabled) {
                        for (Addr *uaddr = addr->FirstUnicastAddress; uaddr != 0; uaddr = uaddr->Next) {
                            ...
                            res.emplace_back(sinaddr_to_asio(reinterpret_cast<sockaddr_in6 *>(uaddr->Address.lpSockaddr)));
                        }
                    }
                }
            } else {
...
#error "..."
        ...
        static std::string get_default_host(const std::string& def_host, const std::string& seg) {
...
#endif //CONTROLSPROG_NETWORK_HPP
