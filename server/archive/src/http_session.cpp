...
http_session(
        tcp::socket&& socket,
        boost::shared_ptr<shared_state> const& state) //, boost::shared_ptr<channel> ch)
...
do_read()
{
    ...
    // Создаем новый парсер для каждого сообщения
    parser_.emplace();

    // Apply a reasonable limit to the allowed size
    // of the body in bytes to prevent abuse.
    // Применяем разумный предел к допустимому размеру
    // тела в байтах для предотвращения злоупотреблений.
...
on_read(beast::error_code ec, std::size_t)
{
    ...
    // Это означает, что они закрыли соединение
    ...
    if(websocket::is_upgrade(parser_->get()))
    {
        ...
        boost::make_shared<websocket_session>(
                ...
                state_)->run(parser_->release()); //, channel_)->run(parser_->release());
        return;
    }
