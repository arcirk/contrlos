...
typedef struct sql_compare_value{
    ...
    [[nodiscard]] std::string to_string(bool quotation_marks = true) const{
        ...
        if(!value.is_array()){
            ...
            result.append(strings::str_sample(template_value, key, strings::str_sample(s_val, v)));
        }else{
            result.append(strings::str_sample(template_value, key, array_to_string(value, quotation_marks)));
        }
        return result;
    }
...
public:
    ...
    query_builder& update(const std::string& table_name, bool use_values = true, bool skip_id = true){
        queryType = Update;
        table_name_ = table_name;
        result = strings::str_sample("update %1% set ", table_name);
        for (auto itr = m_list.cbegin(); itr != m_list.cend() ; ++itr) {
            ...
            if(use_values){
                ...
                if(value.empty())
                    ...
                    result.append(strings::str_sample("='%1%'", value));
            }else
                result.append("=?");
            if(itr != (--m_list.cend())){
                result.append(",\n");
    ...
    query_builder& insert(const std::string& table_name, bool use_values = true, bool skip_id = true){
        queryType = Insert;
        table_name_ = table_name;
        result = strings::str_sample("insert into %1% (", table_name);
        ...
        for (auto itr = m_list.cbegin(); itr != m_list.cend() ; ++itr) {
            ...
            if(use_values){
                ...
                if(value.empty())
                    ...
                    string_values.append(strings::str_sample("'%1%'", value));
            }else
                string_values.append("?");
    ...
    query_builder& union_all(const query_builder& nestedQuery, bool all = true){
        result.append(strings::str_sample("\nunion%1%\n", all ? " all" : ""));
    ...
    [[nodiscard]] std::string prepare(const json& where_not_exists = {}, bool use_values = false) const{
        if (where_not_exists.empty())
            ...
            if(queryType == Insert){
                ...
                return strings::str_sample(query_sample, where_select, tmp);
            }
        }
        ...
        while (rs.next()) {
            ...
            for (int i = 0; i < row.count(); ++i) {
                ...
                if(val.userType() == QMetaType::QString)
                    ...
                    user_data[column_name] = "<бинарные данные>";
            }
            m_rows += user_data;
        }
    ...
    std::string with_recursive(const std::string& table, const sql_values& values, const nlohmann::json& where_root, const nlohmann::json& where_childs, const std::string& order = ""){
        ...
        for (auto itr = values.begin(); itr != values.end(); ++itr) {
            ...
            col.value = itr->field_name == "ref" ? "parent" : 0;
