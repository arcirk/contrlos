...
void TreeViewWidget::setModel(QAbstractItemModel *model)
{
    ...
    if(model->property("typeName").toString() != "TreeSortModel"){
        ...
        connect((TreeItemModel*)model, &TreeItemModel::fetch, this, &TreeViewWidget::onTreeFetch);
    }
...
void TreeViewWidget::setTableToolBar(TreeToolBar *value)
{
    m_toolBar = value;
    connect(m_toolBar, &TreeToolBar::itemClicked, this, &TreeViewWidget::onToolBarItemClicked);
    auto model = get_model();
    if(!model || !m_toolBar)
        return;
...
TreeToolBar *TreeViewWidget::toolBar() const
{
    return m_toolBar;
}
